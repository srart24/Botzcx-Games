...
const premium = JSON.parse(fs.readFileSync("./data/database/premium.json"))
...
const addPremiumUser = (userId, expired, _dir) => {
	...
	fs.writeFileSync("./data/database/premium.json", JSON.stringify(_dir))
}
...
const expiredCheck = (srart24, msg, _dir) => {
	setInterval(() => {
		let position = null
		Object.keys(_dir).forEach((i) => {
		...
		if (position !== null) {
			...
			fs.writeFileSync("./data/database/premium.json", JSON.stringify(_dir))
			idny ? srart24.sendMessage(idny, {text: "Premium Anda Sudah Berakhir"}) : ""
			idny = false
		}
	}, 1000)
}
